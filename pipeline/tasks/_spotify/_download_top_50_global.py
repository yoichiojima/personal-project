...
def download_top_50_global():
    ...
    df = df[["added_at", "track"]]

    df = df.copy()
    df["track_id"] = df["track"].apply(lambda row: row["id"])

    df["track_name"] = df["track"].apply(lambda row: row["name"])
    df["artists_id"] = df["track"].apply(
        lambda row: [artist["id"] for artist in row["artists"]]
    )
    df["artists_name"] = df["track"].apply(
        lambda row: [artist["name"] for artist in row["artists"]]
    )

    df = df[["track_name", "artists_name", "added_at", "track_id", "artists_id"]]
